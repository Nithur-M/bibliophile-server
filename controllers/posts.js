...
export const getPosts = async (req, res) => {
  ...
  try {
    const shelf = await Shelf.findOne({ uid: "AR0NZU5PnEVx4TyimR8fGM3rctU2" });
    const readBooks = shelf.readBooks.map(({ title: bookTitle }) => ({ bookTitle }));

    const bookRecommendationsRes = await fetch("https://biblio-book-rec.azurewebsites.net/predict", {
      method: 'POST',
